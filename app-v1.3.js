...
const APP_BUILD = "plc-tools-separated-float-labels";
...
// ===== PLC address conversion =====
function parsePlcAddress(raw, base){
...
function formatPlcAddress(prefix, value, base){
...
function calcPlcAddress(){
  ...
  const baseFrom=parsePlcAddress($("plcBaseFrom").value,fromBase);
  const baseTo=parsePlcAddress($("plcBaseTo").value,toBase);
  const targetFrom=parsePlcAddress($("plcTargetFrom").value,fromBase);

  if(!baseFrom || !baseTo || !targetFrom){
    $("plcAddressResult").textContent="—";
    $("plcAddressDetail").textContent="アドレス形式または進数を確認してください。";
    return;
  }

  if(baseFrom.prefix!==targetFrom.prefix){
    $("plcAddressResult").textContent="—";
    $("plcAddressDetail").textContent="基準元と求めたい元のデバイス種別を合わせてください。";
    return;
  }

  const offset=targetFrom.value-baseFrom.value;
  const resultValue=baseTo.value+offset;
  const result=formatPlcAddress(baseTo.prefix,resultValue,toBase);

  if(result===null){
    $("plcAddressResult").textContent="—";
    $("plcAddressDetail").textContent="変換先アドレスが0未満になるため計算できません。";
    return;
  }

  const sign=offset>=0?"+":"−";
  const abs=Math.abs(offset);
  const offsetInFrom=abs.toString(fromBase).toUpperCase();
  ...
  $("plcAddressDetail").textContent=
    ...
    + ` / ${targetFrom.prefix}${targetFrom.digits} は ${sign}${abs}点`
    + `（${fromBase===10?"10進":fromBase===16?"16進":"8進"} ${sign}${offsetInFrom}）`
    + ` → ${result}`;
}
...
["plcFromBase","plcToBase"].forEach(id=>{
  $(id).addEventListener("change",calcPlcAddress);
});

// ===== PLC data type / occupied address =====
function calcDataAddressUsage(){
  const base=parseInt($("dataAddressBase").value,10);
  const parsed=parsePlcAddress($("dataStartAddress").value,base);

  if(!parsed){
    $("dataAddressUsage").textContent="—";
    $("dataAddressDetail").textContent="開始アドレスまたは進数を確認してください。";
    ["dataWord0","dataWord1","dataWord2","dataWord3"].forEach(id=>$(id).textContent="—");
    return;
  }

  const type=$("dataTypeSize").value;
  const words=type==="1" ? 1 : (type==="4" || type==="4f") ? 4 : 2;
  const addresses=[];

  for(let i=0;i<words;i++){
    const a=formatPlcAddress(parsed.prefix,parsed.value+i,base);
    addresses.push(a);
  }

  const next=formatPlcAddress(parsed.prefix,parsed.value+words,base);
  const usage=words===1 ? addresses[0] : `${addresses[0]} ～ ${addresses[addresses.length-1]}`;

  $("dataAddressUsage").textContent=usage;
  const typeLabel=$("dataTypeSize").selectedOptions[0].textContent;
  $("dataAddressDetail").textContent=`${typeLabel} / ${words}ワード使用 / 次の空きアドレス ${next}`;

  ["dataWord0","dataWord1","dataWord2","dataWord3"].forEach((id,i)=>{
    $(id).textContent=i<addresses.length ? addresses[i] : "—";
  });
}

$("dataStartAddress").addEventListener("input",calcDataAddressUsage);
["dataAddressBase","dataTypeSize"].forEach(id=>{
...
if("serviceWorker" in navigator && location.protocol!=="file:"){
  ...
  window.addEventListener("load",async()=>{
    try{
      ...
      const reg=await navigator.serviceWorker.register("service-worker-v1.3.js",{updateViaCache:"none"});
      await reg.update();
