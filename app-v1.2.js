...
const APP_BUILD = "plc-data-address-usage";
...
function calcDataAddressUsage(){
  ...
  const words=type==="1" ? 1 : type==="4" ? 4 : 2;
  const addresses=[];
  ...
  $("dataAddressDetail").textContent=`${words}ワード使用 / 次の空きアドレス ${next}`;
...
if("serviceWorker" in navigator && location.protocol!=="file:"){
  ...
  window.addEventListener("load",async()=>{
    try{
      ...
      const reg=await navigator.serviceWorker.register("service-worker-v1.2.js",{updateViaCache:"none"});
      await reg.update();
