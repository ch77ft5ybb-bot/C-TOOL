...
const APP_BUILD = "plc-bit-word-storage-rework";
...
// ===== PLC address / bit allocation =====
function parsePlcSimpleAddress(raw, base){
  const text=String(raw||"").trim().toUpperCase().replace(/\s+/g,"");
  const m=text.match(/^([A-Z]+)([0-9A-F]+)$/);
  if(!m) return null;
  const prefix=m[1], digits=m[2];
  const patterns={16:/^[0-9A-F]+$/,10:/^[0-9]+$/,8:/^[0-7]+$/};
  if(!patterns[base] || !patterns[base].test(digits)) return null;
  const value=parseInt(digits,base);
  if(!Number.isSafeInteger(value)) return null;
  return {prefix,digits,value};
}

function parsePlcWordBitAddress(raw, base){
  const text=String(raw||"").trim().toUpperCase().replace(/\s+/g,"");
  const m=text.match(/^([A-Z]+)([0-9A-F]+)(?:\.([0-9A-F]))?$/);
  if(!m) return null;
  const prefix=m[1], digits=m[2], bitText=m[3] ?? "0";
  const patterns={16:/^[0-9A-F]+$/,10:/^[0-9]+$/,8:/^[0-7]+$/};
  if(!patterns[base] || !patterns[base].test(digits)) return null;
  if(!/^[0-9A-F]$/.test(bitText)) return null;
  const value=parseInt(digits,base);
  const bit=parseInt(bitText,16);
  if(!Number.isSafeInteger(value) || bit<0 || bit>15) return null;
  return {prefix,digits,value,bit};
}

function formatPlcSimple(prefix,value,base){
  if(!Number.isSafeInteger(value) || value<0) return null;
  return prefix + value.toString(base).toUpperCase();
}

function formatPlcWordBit(prefix,word,bit,base){
  if(!Number.isSafeInteger(word) || word<0 || bit<0 || bit>15) return null;
  return prefix + word.toString(base).toUpperCase() + "." + bit.toString(16).toUpperCase();
}

function calcPlcAddress(){
  const mode=$("plcMapMode").value;
  const fromBase=parseInt($("plcFromBase").value,10);
  const toBase=parseInt($("plcToBase").value,10);
  const baseFrom=parsePlcSimpleAddress($("plcBaseFrom").value,fromBase);
  const targetFrom=parsePlcSimpleAddress($("plcTargetFrom").value,fromBase);

  if(!baseFrom || !targetFrom || baseFrom.prefix!==targetFrom.prefix){
    $("plcAddressResult").textContent="—";
    $("plcAddressDetail").textContent="変換元アドレス・進数・デバイス種別を確認してください。";
    return;
  }

  const offset=targetFrom.value-baseFrom.value;

  if(mode==="bitToWord"){
    const baseTo=parsePlcWordBitAddress($("plcBaseTo").value,toBase);
    if(!baseTo){
      $("plcAddressResult").textContent="—";
      $("plcAddressDetail").textContent="変換先は D1000 または D1000.0 のように入力してください。";
      return;
    }

    const totalBit=baseTo.bit+offset;
    const wordDelta=Math.floor(totalBit/16);
    const bit=((totalBit%16)+16)%16;
    const word=baseTo.value+wordDelta;
    const result=formatPlcWordBit(baseTo.prefix,word,bit,toBase);

    if(result===null){
      $("plcAddressResult").textContent="—";
      $("plcAddressDetail").textContent="変換結果が負アドレスになるため計算できません。";
      return;
    }

    const baseToText=formatPlcWordBit(baseTo.prefix,baseTo.value,baseTo.bit,toBase);
    const sign=offset>=0?"+":"";
    $("plcAddressResult").textContent=result;
    $("plcAddressDetail").textContent=
      `${baseFrom.prefix}${baseFrom.digits} → ${baseToText}`
      + ` / ${targetFrom.prefix}${targetFrom.digits} は ${sign}${offset}点 → ${result}`;
    return;
  }

  const baseTo=parsePlcSimpleAddress($("plcBaseTo").value,toBase);
  if(!baseTo){
    $("plcAddressResult").textContent="—";
    $("plcAddressDetail").textContent="同一オフセット変換では、変換先を M1000 のように入力してください。";
    return;
  }
  const result=formatPlcSimple(baseTo.prefix,baseTo.value+offset,toBase);
  if(result===null){
    $("plcAddressResult").textContent="—";
    $("plcAddressDetail").textContent="変換結果が負アドレスになるため計算できません。";
    return;
  ...
  $("plcAddressResult").textContent=result;
  $("plcAddressDetail").textContent=
    `${baseFrom.prefix}${baseFrom.digits} → ${baseTo.prefix}${baseTo.digits}`
    + ` / ${targetFrom.prefix}${targetFrom.digits} は ${sign}${offset}点 → ${result}`;
}

["plcBaseFrom","plcBaseTo","plcTargetFrom"].forEach(id=>$(id).addEventListener("input",calcPlcAddress));
["plcMapMode","plcFromBase","plcToBase"].forEach(id=>$(id).addEventListener("change",calcPlcAddress));

// ===== PLC data type / storage visualization =====
const PLC_DATA_TYPES={
  i16:{label:"16bit整数（INT）",bits:16,words:1,kind:"signed"},
  u16:{label:"16bit符号なし（UINT）",bits:16,words:1,kind:"unsigned"},
  i32:{label:"32bit整数（DINT）",bits:32,words:2,kind:"signed"},
  u32:{label:"32bit符号なし（UDINT）",bits:32,words:2,kind:"unsigned"},
  f32:{label:"単精度浮動小数点（REAL / FLOAT）",bits:32,words:2,kind:"float32"},
  i64:{label:"64bit整数（LINT）",bits:64,words:4,kind:"signed"},
  u64:{label:"64bit符号なし（ULINT）",bits:64,words:4,kind:"unsigned"},
  f64:{label:"倍精度浮動小数点（LREAL / DOUBLE）",bits:64,words:4,kind:"float64"}
};

function parseIntegerBigInt(text){
  const s=String(text||"").trim();
  if(!/^[+-]?(?:\d+|0[xX][0-9a-fA-F]+)$/.test(s)) return null;
  try{return BigInt(s);}catch{return null;}
}

function integerToUnsignedBits(text,bits,signed){
  const v=parseIntegerBigInt(text);
  if(v===null) return null;
  const B=BigInt(bits);
  const mod=1n<<B;
  if(signed){
    const min=-(1n<<(B-1n)), max=(1n<<(B-1n))-1n;
    if(v<min || v>max) return null;
    return v<0 ? mod+v : v;
  }else{
    if(v<0n || v>=mod) return null;
    return v;
  }
}

function float32Bits(text){
...
function packedBitsForType(type,text){
  const def=PLC_DATA_TYPES[type];
  if(!def) return null;
  if(def.kind==="signed") return integerToUnsignedBits(text,def.bits,true);
  if(def.kind==="unsigned") return integerToUnsignedBits(text,def.bits,false);
  if(def.kind==="float32") return float32Bits(text);
  if(def.kind==="float64") return float64Bits(text);
  return null;
}

function wordHex(word){ return word.toString(16).toUpperCase().padStart(4,"0"); }
function wordBin(word){ return word.toString(2).padStart(16,"0").match(/.{4}/g).join(" "); }

function wordOnBitNumbers(word){
  const bits=[];
  for(let b=0;b<16;b++){
    if(word & (1<<b)) bits.push(b);
  }
  return bits;
}

function calcDataAddressUsage(){
  const base=parseInt($("dataAddressBase").value,10);
  const addr=parsePlcSimpleAddress($("dataStartAddress").value,base);
  const type=$("dataTypeSize").value;
  const def=PLC_DATA_TYPES[type];
  const packed=packedBitsForType(type,$("dataValue").value);

  if(!addr || !def){
    $("dataAddressUsage").textContent="—";
    $("dataAddressDetail").textContent="開始アドレス・進数・データ型を確認してください。";
  ...
  for(let i=0;i<def.words;i++){
    addresses.push(formatPlcSimple(addr.prefix,addr.value+i,base));
  }
  const next=formatPlcSimple(addr.prefix,addr.value+def.words,base);
  $("dataAddressUsage").textContent=def.words===1 ? addresses[0] : `${addresses[0]} ～ ${addresses[addresses.length-1]}`;
  ...
  const hexDigits=def.bits/4;
  $("dataPackedHex").textContent="0x"+packed.toString(16).toUpperCase().padStart(hexDigits,"0");
  $("dataPackedInfo").textContent=`${def.bits}bit / ${def.label}`;

  // Split the packed value into 16-bit words, low-order word first.
  const lowFirstWords=[];
  for(let i=0;i<def.words;i++){
    lowFirstWords.push(Number((packed >> BigInt(i*16)) & 0xFFFFn));
  }
  const storedWords=$("dataWordOrder").value==="lowFirst"
    ? lowFirstWords
    : [...lowFirstWords].reverse();

  const allOnBits=[];
  const rows=storedWords.map((word,i)=>{
    const on=wordOnBitNumbers(word);
    const bitAddresses=on.map(b=>`${addresses[i]}.${b.toString(16).toUpperCase()}`);
    allOnBits.push(...bitAddresses);
    return `<tr>`
      + `<td>${addresses[i]}</td>`
      + `<td>${wordHex(word)}</td>`
      + `<td class="mono-cell">${wordBin(word)}</td>`
      + `<td>${bitAddresses.length?bitAddresses.join(", "):"—"}</td>`
      + `</tr>`;
  }).join("");

  $("dataWordRows").innerHTML=rows;
  $("dataOnBits").textContent=allOnBits.length ? allOnBits.join(", ") : "ONビットなし";
  $("dataBitNote").textContent=
    $("dataWordOrder").value==="lowFirst"
      ? "先頭アドレスに下位ワードを格納 / ビット番号 .0 ～ .F"
      : "先頭アドレスに上位ワードを格納 / ビット番号 .0 ～ .F";
}

["dataStartAddress","dataValue"].forEach(id=>$(id).addEventListener("input",calcDataAddressUsage));
["dataAddressBase","dataTypeSize","dataWordOrder"].forEach(id=>$(id).addEventListener("change",calcDataAddressUsage));
...
if("serviceWorker" in navigator && location.protocol!=="file:"){
  ...
  window.addEventListener("load",async()=>{
    try{
      ...
      const reg=await navigator.serviceWorker.register("service-worker-v1.4.js",{updateViaCache:"none"});
      await reg.update();
