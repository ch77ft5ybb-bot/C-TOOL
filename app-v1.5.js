...
const APP_BUILD = "plc-duplicate-id-live-fix";
...
// ===== PLC tools v1.5 =====
// Device address notation is defined automatically.
// X / Y / W: hexadecimal; M / D / R / ZR: decimal.
const PLC_DEVICE_BASES={X:16,Y:16,W:16,M:10,D:10,R:10,ZR:10};

function plcBase(prefix){
  return PLC_DEVICE_BASES[String(prefix||"").toUpperCase()] || 10;
}

function parsePlcAddress(raw, allowBit=false){
  ...
  const bit=allowBit && m[3]!==undefined ? parseInt(m[3],16) : 0;

  if(!Number.isSafeInteger(value) || bit<0 || bit>15) return null;
  return {prefix,value,bit};
}

function formatPlcAddress(prefix,value){
  if(!Number.isSafeInteger(value) || value<0) return null;
  return prefix + value.toString(plcBase(prefix)).toUpperCase();
}

function formatPlcBitAddress(prefix,value,bit){
  const a=formatPlcAddress(prefix,value);
  if(a===null || bit<0 || bit>15) return null;
  return a + "." + bit.toString(16).toUpperCase();
}

function calcPlcAddress(){
  const mode=$("plcMapMode").value;
  const baseFrom=parsePlcAddress($("plcBaseFrom").value);
  const targetFrom=parsePlcAddress($("plcTargetFrom").value);

  if(!baseFrom || !targetFrom || baseFrom.prefix!==targetFrom.prefix){
    $("plcAddressResult").textContent="—";
    $("plcAddressDetail").textContent="基準元と求めたい変換元を確認してください。";
    return;
  }
  ...
  if(mode==="bitToWord"){
    const baseTo=parsePlcAddress($("plcBaseTo").value,true);
    if(!baseTo){
      $("plcAddressResult").textContent="—";
    ...
    const result=formatPlcBitAddress(baseTo.prefix,baseTo.value+wordDelta,bit);

    $("plcAddressResult").textContent=result || "—";
    $("plcAddressDetail").textContent=result
      ? `${formatPlcAddress(baseFrom.prefix,baseFrom.value)} → ${formatPlcBitAddress(baseTo.prefix,baseTo.value,baseTo.bit)} / ${formatPlcAddress(targetFrom.prefix,targetFrom.value)} は ${offset>=0?"+":""}${offset}点 → ${result}`
      : "変換結果が負アドレスです。";
    return;
  }

  const baseTo=parsePlcAddress($("plcBaseTo").value);
  if(!baseTo){
    $("plcAddressResult").textContent="—";
  ...
  const result=formatPlcAddress(baseTo.prefix,baseTo.value+offset);
  $("plcAddressResult").textContent=result || "—";
  $("plcAddressDetail").textContent=result
    ? `${formatPlcAddress(baseFrom.prefix,baseFrom.value)} → ${formatPlcAddress(baseTo.prefix,baseTo.value)} / ${formatPlcAddress(targetFrom.prefix,targetFrom.value)} は ${offset>=0?"+":""}${offset}点 → ${result}`
    : "変換結果が負アドレスです。";
}
...
function calcDataAddressUsage(){
  const start=parsePlcAddress($("dataStartAddress").value);
  ...
  const addresses=Array.from(
    {length:def.words},
    (_,i)=>formatPlcAddress(start.prefix,start.value+i)
  );
  const next=formatPlcAddress(start.prefix,start.value+def.words);

  $("dataAddressUsage").textContent=def.words===1
    ? addresses[0]
    : `${addresses[0]} ～ ${addresses[def.words-1]}`;
  $("dataAddressDetail").textContent=
    `${def.label} / ${def.words}ワード使用 / 次の空き ${next}`;
  ...
  $("dataPackedHex").textContent=
    "0x"+packed.toString(16).toUpperCase().padStart(def.bits/4,"0");
  ...
  $("dataWordRows").innerHTML=words.map((word,i)=>{
    ...
    return `<tr>`
      + `<td>${addresses[i]}</td>`
      + `<td>${hex}</td>`
      + `<td class="mono-cell">${bin}</td>`
      + `<td>${bits.length?bits.join(", "):"—"}</td>`
      + `</tr>`;
  }).join("");

  $("dataOnBits").textContent=allOn.length
    ? allOn.join(", ")
    : "ONビットなし";
...
// iPhone/Safariでも入力中と確定時の両方で再計算。
["plcBaseFrom","plcBaseTo","plcTargetFrom"].forEach(id=>{
  $(id).addEventListener("input",calcPlcAddress);
  $(id).addEventListener("change",calcPlcAddress);
});
$("plcMapMode").addEventListener("change",calcPlcAddress);
...
if("serviceWorker" in navigator && location.protocol!=="file:"){
  ...
  window.addEventListener("load",async()=>{
    try{
      ...
      const reg=await navigator.serviceWorker.register("service-worker-v1.5.js",{updateViaCache:"none"});
      await reg.update();
