...
const APP_BUILD = "plc-source-device-sync";
...
function calcPlcAddress(){
  ...
  const targetDev=fromDev;
  $("plcTargetFromDeviceDisplay").textContent=`${fromDev}（基準元と連動）`;
...
["plcBaseFromDevice","plcBaseToDevice","plcMapMode"].forEach(id=>{
  $(id).addEventListener("change",calcPlcAddress);
});
...
if("serviceWorker" in navigator && location.protocol!=="file:"){
  ...
  window.addEventListener("load",async()=>{
    try{
      ...
      const reg=await navigator.serviceWorker.register("service-worker-v1.8.js",{updateViaCache:"none"});
      await reg.update();
